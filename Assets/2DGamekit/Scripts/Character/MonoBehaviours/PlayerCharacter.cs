...
namespace hgcxt
{
    ...
    public class PlayerCharacter : MonoBehaviour
    {
        ...
        protected bool m_InMenus = false;
        ...
        void Update()
        {
            ...
            if (PlayerInput.Instance.Pause.Down)
            {
                ...
                OpenUIByKeyboard("Pause");

            }
            //系统设置——Esc
            if (PlayerInput.Instance.Option.Down)
            {
                OpenUIByKeyboard("esc");
            ...
            {
                print(m_InInventory);
                OpenUIByKeyboard("Inventory");
               
            }
            //线索图——J
            if (PlayerInput.Instance.ClueMap.Down)
            {
             
                OpenUIByKeyboard("ClueMap");
        ...
        public void Unmenus()
        {
            ...
            if (Time.timeScale > 0)
                return;
            StartCoroutine(UnmenusCoroutine());
        }
        ...
        public void Uninventory()
        {
         ...
         /*   if (Time.timeScale > 0)
            ...
            StartCoroutine(UninventoryCoroutine());
        }

        protected IEnumerator UninventoryCoroutine()
        {
            //Time.timeScale = 1;
            UnityEngine.SceneManagement.SceneManager.UnloadSceneAsync("Inventory");
            PlayerInput.Instance.GainControl();
            //we have to wait for a fixed update so the pause button state change, otherwise we can get in case were the update
            //of this script happen BEFORE the input is updated, leading to setting the game in pause once again
            yield return new WaitForFixedUpdate();
        ...
        /*———————————————线索———————————————*/
        public void Uncluemap()
        {
            //if the timescale is already > 0, we 
           /* if (Time.timeScale > 0)
                return;*/

            StartCoroutine(UncluemapCoroutine());
        }

        protected IEnumerator UncluemapCoroutine()
        {
//            Time.timeScale = 1;
            UnityEngine.SceneManagement.SceneManager.UnloadSceneAsync("ClueMap");
            print("关门");
            PlayerInput.Instance.GainControl();
            //we have to wait for a fixed update so the pause button state change, otherwise we can get in case were the update
            //of this script happen BEFORE the input is updated, leading to setting the game in pause once again
       ...
       //水平移动
        ...
        public float Accelerate()
        {
            ...
            float speedScale =  PlayerInput.Instance.Accelerate.Held? 2f : 1f;
            return speedScale;
        }
...
*/
        ...
        public void CheckForPushing()
        {
            ...
            if (m_CurrentPushables.Count > 0)
            {
                ...
                bool movingUp = PlayerInput.Instance.Vertical.Value > float.Epsilon;
                bool movingDown = PlayerInput.Instance.Vertical.Value < -float.Epsilon;
                ...
                if (pushableOnCorrectSide)
                {
                    ...
                    moveToPosition.y = m_CharacterController2D.transform.position.y;
...
*/
        ...
        public bool CheckForAccelerate()
        {
            return PlayerInput.Instance.Accelerate.Down;
        }
        ...
        {
            Vector2 colliderBottom = m_CharacterController2D.transform.position;
            colliderBottom = colliderBottom + m_Capsule.offset + Vector2.down * m_Capsule.size.y * 0.5f;
...
/*
                        ...
                        PlayerInput.Instance.ClueMap.GainControl();*/
                        m_InClueMap = true;
                       // Time.timeScale = 0;
                        UnityEngine.SceneManagement.SceneManager.LoadSceneAsync("ClueMap", UnityEngine.SceneManagement.LoadSceneMode.Additive);
                    }
...
/*
                        PlayerInput.Instance.ReleaseControl(false);
                        PlayerInput.Instance.Inventory.GainControl();*/
                        m_InInventory = true;
                        //Time.timeScale = 0;
                        UnityEngine.SceneManagement.SceneManager.LoadSceneAsync("Inventory", UnityEngine.SceneManagement.LoadSceneMode.Additive);
                    }
                ...
                case "esc":
                    if (!m_InMenus)
                    {
                        ...
                        PlayerInput.Instance.ReleaseControl(false);
                        PlayerInput.Instance.Option.GainControl();
                        m_InMenus = true;
                        Time.timeScale = 0;
                        UnityEngine.SceneManagement.SceneManager.LoadSceneAsync("Option", UnityEngine.SceneManagement.LoadSceneMode.Additive);
                    }
                    else
                    {
                        Unmenus();
                ...
                case "Pause":
                    if (!m_InPause)
                    {
                         ...
                         m_InPause = true;
                         Time.timeScale = 0;
                         UnityEngine.SceneManagement.SceneManager.LoadSceneAsync("Pause", UnityEngine.SceneManagement.LoadSceneMode.Additive);
